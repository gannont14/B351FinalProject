...
class ContextoSolver:
    def __init__(self, webDriver="Firefox", randomGame=True):
        ...
        try:
            ...
            # Narrow initial word list
            all_words = [
                w.lower() for w in words.words()
                if len(w) > 2 and w.isalpha() and not w.endswith("ly")
            ]

            # Apply POS tagging and filter for nouns
            tagged_words = pos_tag(all_words)
            self.english_words = set(
                word for word, tag in tagged_words if tag.startswith("NN")
            )

            logging.info(f"Loaded {len(self.english_words)} nouns into the word bank.")
            logging.info(f"Sample nouns: {list(self.english_words)[:10]}")  # Debugging
        except Exception as e:
            logging.error(f"Error loading and filtering English words: {e}")

            # Fallback to a basic common words list
            fallback_word_list = api.load('word2vec-google-news-300').index_to_key[:50000]
            tagged_fallback_words = pos_tag(fallback_word_list)
            self.english_words = set(
                word for word, tag in tagged_fallback_words if tag.startswith("NN")
            )
