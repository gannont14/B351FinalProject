...
class ContextoSolver:
    ...
    def log_results(self, game_number, guess_count, winning_guess, failed=False):
        ...
        with open(self.api_log_file, "a") as f:
            f.write(
                f"{time.strftime('%Y-%m-%d %H:%M:%S')},{game_number},{guess_count},{winning_guess},{status},{self.model_number}\n")
    ...
    def make_diverse_guesses(self, candidates, n_clusters=5, words_per_cluster=2):
        if not candidates:
            return []
        ...
        if len(words) < n_clusters * words_per_cluster:
            return words
        ...
        try:
            ...
            logging.debug(f"Found {len(vectors)} valid vectors out of {len(words)} candidates.")

            if not vectors:
                logging.warning("No valid vectors found in candidates.")
            ...
            logging.debug(f"Selected {len(diverse_guesses)} diverse guesses.")
...
def main():
    ...
    if len(sys.argv) > 1:
        try:
            gameNumber = int(sys.argv[1])
        except ValueError:
            print("Invalid game number passed. Using None.")
            gameNumber = None

    solver = ContextoSolver(webDriver="API", gameNumber=gameNumber)
    guesses = -1
    try:
